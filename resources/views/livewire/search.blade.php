<div class="inline-block relative" x-data="{ open: true }" >
    <input @click.away="{ open = false; $this.resetIndex(); }" @click="{ open = true }" class="bg-gray-200 text-gray-700 border-2 focus:outline-none placeholder-gray-500 px-2 py-1 
    rounded-full mr-3 w-56" 
    placeholder="Rechercher mission" wire:model="query"
        ...
        x-show="open">
            @if(count($jobs) > 0)
                @foreach($jobs as $index => $job)
                    <p class="p-1 {{ $index === $selectedIndex ? 'text-green-500' : '' }}">{{ $job->titre }}</p>
                @endforeach
            @else
